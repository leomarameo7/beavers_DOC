...
Downstream DOC was set primarily by the upstream state it inherits: the inheritance slope was 0.94 (95\% BCI: 0.89 ; 0.99 mg per mg), the largest path in the model (Figure~\ref{fig:2}). Beaver-engineering added a local modification through the primary producers and the soil litter: phytoplankton (standardized effect +0.14) and macrophytes (+0.09) both increased downstream DOC, as did soil litter cover (+0.07). Solar radiation had a positive influence on both primary producer groups (macrophytes +0.15, phytoplankton +0.18), and water residence time had a further positive effect on phytoplankton (+0.11), itself increased by the number of dams (+0.07) and decreased by channel gradient (-0.08). We found no evidence that maximum dam height affected water residence time, or that water residence time directly altered downstream DOC (Figure~\ref{fig:2}). Beaver-engineering therefore does not retain the DOC it receives from upstream; it inherits it, then adds to it through the hydrological and biological pathways it creates. This reparameterization, with downstream DOC (rather than \(\Delta\)DOC) as the outcome, is mathematically equivalent to the \(\Delta\)DOC formulation used for Q1 (Methods); a series of structural and prior sensitivity analyses confirmed that these mediator effects do not depend on the assumed causal structure or on the informativeness of the priors (Methods).
