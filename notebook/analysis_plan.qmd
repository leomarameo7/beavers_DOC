...
**Q3 — Magnitude.** *How much does beaver-engineering add or remove DOC, relative to the inherited seasonal contrast?*
Hypothesis: if the season is summer, then the relative response $R_{summer}$ will be positive and a substantial fraction of the inherited seasonal contrast, because higher light and temperature raise autochthonous DOC production inside the pond independently of the upstream corridor (Mulholland & Hill 1997; Battin et al. 2023); if the season is winter, then $R_{winter}$ will be close to zero because primary production is minimal. Because $A_\text{beaver}$ is a very small part of $A_\text{floodplain}$ (footprint contrast $F \gg 1$), this response arises on a footprint far smaller than the corridor that generates $S$.
...
| Q3 Magnitude | $R_s$, with $F$ as context | @fig-R | Figure 2 (main text) |
...
$R_s$ is computed from the **latent** concentrations of a Bayesian hierarchical measurement-error model (one model per measured concentration, log scale, 0.2 mg L$^{-1}$ instrument error; `notebook/latent_concentrations_fit.R`), so the ratio is never formed from raw values and its uncertainty is carried through (`r nrow(readRDS(here("results","latent_R_thin.rds"))$S)` posterior draws per stream).
...
2.  **Results.** The amplification-factor section and its figure are removed; Q3 is answered by $R_s$ (Figure 2) with the footprint sentence above.
3.  **Methods.** The definitions of $S$, $R_s$ and $F$ and the latent-concentration model replace the amplification-factor equations; Figure 3b is redrawn.
4.  **Appendix S1.** Figure S10 (derivation of the variables) is added.
:::
...
```{r session}
#| echo: false
cat("Data: data/processed/m12.csv | latent concentrations: notebook/latent_concentrations_fit.R -> results/latent_R.rds (thinned copy latent_R_thin.rds is tracked)\n")
