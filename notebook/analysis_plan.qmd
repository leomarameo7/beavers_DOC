...
![**Drivers of downstream DOC.** Structural causal model with downstream DOC as the outcome (model `b1_downstream`) and the posterior mean standardized effects; the upstream-DOC path is the inheritance slope (0.95, mg per mg). Skyblue: positive and coral: negative effects with posterior probability above 0.90; dashed grey: no-evidence. Drawn by `notebook/figure_1b_downstream.R`.](../images/Figure_1b_downstream.png){#fig-q2}
