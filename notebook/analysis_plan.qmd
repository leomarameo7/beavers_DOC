...
ctr <- tibble(
  Contrast = c("Reverses depletion→enrichment  −  reverses enrichment→depletion",
               "Amplifies enrichment  −  dampens enrichment",
               "Number of reaches with B > 0 (out of 158)",
               "Median B across reaches (mg L⁻¹)"),
...
glue::glue(
"Reaches whose six-state label carries posterior probability > 0.9: {sum(p_state>0.9)} of {nrow(d)} ({round(100*mean(p_state>0.9))}%)
Reaches with P(|B| > 0.5 mg/L) > 0.9, i.e. a practically relevant seasonal modification: {sum(d$B_strong)} of {nrow(d)}
