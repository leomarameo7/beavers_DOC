...
vars <- tribble(
  ...
  "the inherited seasonal contrast the upstream river corridor delivers to the reach; $S > 0$ seasonal enrichment, $S < 0$ seasonal depletion **(mg L$^{-1}$)**",
  ...
  "$T$", "downstream seasonal contrast", "$S + B$ (exact identity)",
  "the inherited seasonal contrast leaving the reach after beaver-engineering has modified it **(mg L$^{-1}$)**",
  ...
  "How large is the beaver-engineered footprint, and how large is the floodplain area that generates the inherited seasonal contrast?",

  "$\\mathbf{G}$", "seasonal **G**ain", "$B / S$",
  "how beaver-engineering modifies the inherited seasonal contrast ($T = S(1 + G)$): $G > 0$ reinforces, $-1 < G < 0$ dampens, $G = -1$ cancels, $G < -1$ reverses it, whether enrichment ($S > 0$) or depletion ($S < 0$) **(dimensionless)**",
  ...
  "how much DOC beaver-engineering adds ($R_s > 0$) or removes ($R_s < 0$) in season $s$, expressed as a fraction of the inherited seasonal contrast; $R_s = 0.5$ means the change across the reach is half the seasonal contrast **(dimensionless)**",
  "How much does beaver-engineering add or remove DOC in season $s$, relative to the inherited seasonal contrast?",
  ...
  "How small is the beaver-engineered footprint relative to the floodplain area that generates the inherited seasonal contrast?",

  "$\\mathbf{L}$", "**L**everage", "$\\lvert G \\rvert \\cdot F$",
  "magnitude of beaver-engineering's modification of the inherited seasonal contrast, per unit of engineered footprint **(dimensionless)**",
  "How strongly does beaver-engineering modify the inherited seasonal contrast relative to the small area it occupies?",
  ...
  "$D_\\text{floodplain}$", "area-normalised seasonal contrast", "$S / A_\\text{floodplain}$",
...
| **Q3 Magnitude** | Irrespective of direction, how large is the beaver-mediated DOC change per unit area relative to the seasonal floodplain-scale variation, and does it reinforce or oppose the inherited seasonal contrast? | The change per unit area is orders of magnitude larger than the floodplain-scale seasonal contrast, and beaver-engineering **modifies** (rather than merely adds to) the inherited seasonal contrast. | $R_s$ (add/remove, relative to the inherited seasonal contrast), $G$ (reinforce/dampen/reverse the inherited seasonal contrast), $F$ (spatial context); $M = \lvert R_s \rvert F$ only as a bridge |
...
d <- inner_join(w, s, by = c("site", "beaver_territory")) |>
  mutate(
    ...
    S       = up_s - up_w,          # inherited seasonal contrast
    B       = ds - dw,              # beaver seasonal modification
    Tt      = dn_s - dn_w,          # downstream seasonal contrast (= S + B)
    G       = B / S,                # seasonal gain
...
## 7.0 NEW: the relative response R_s — how much beaver-engineering adds or removes, relative to the inherited seasonal contrast

$R_s = \Delta\text{DOC}_s / \lvert S \rvert$ keeps the sign of the beaver effect (add or remove) and expresses its size as a fraction of the inherited seasonal contrast. It is estimated from the **latent** concentrations of a hierarchical measurement-error model (one model per measured concentration, log scale, 0.2 mg L$^{-1}$ error), so the ratio never divides by a raw value.
...
## 7.2 NEW: does beaver-engineering reinforce, dampen or reverse the inherited seasonal contrast?
...
Between-reach SD of the true inherited seasonal contrast S: {round(mean(as_draws_df(fS)$sd_reach__Intercept),2)} mg/L (measurement SD 0.283), so shrinkage is small")
```
...
pa <- ggplot(d, aes(S_pm, B_pm)) +
  ...
  labs(x = expression("inherited seasonal contrast   S   (mg "*L^-1*")"),
...
tb <- tab |> mutate(state = fct_rev(state),
                    side = ifelse(state %in% STATES[c(1, 6)], "pushes toward summer enrichment",
                                  ifelse(state %in% STATES[c(3, 4)], "pushes toward summer depletion",
                                         "dampens the inherited seasonal contrast")))
pb <- ggplot(tb, aes(y = state)) +
  ...
  scale_colour_manual(values = c("pushes toward summer enrichment" = OK[["blue"]],
                                 ...
                                 "dampens the inherited seasonal contrast"    = "grey40"), name = NULL) +
...
**(a)** Beaver-engineered reaches occupy all six response states: they reinforce, dampen and occasionally reverse the inherited seasonal contrast they inherit. This is the visual proof that the reach is an **operator on an incoming signal**, not a fixed source or sink.
...
**Beaver-engineering systematically pushes the inherited seasonal contrast toward summer enrichment.** This is the same signal as Q1 (+0.14 mg L⁻¹ in summer) and the primary-producer pathway of Q2, now expressed as an operation on the incoming signal rather than as an average.
